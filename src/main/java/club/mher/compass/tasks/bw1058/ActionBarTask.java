package club.mher.compass.tasks.bw1058;
...
public class ActionBarTask extends BukkitRunnable {
    ...
    @Override
    public void run() {
        if (arena.getPlayers().size() <= 1 || !BW1058.getTrackingArenaMap().containsKey(arena) || BW1058.getTrackingTeamMap(arena) == null) {
            this.cancel();
            return;
        }
        for (Map.Entry<UUID, ITeam> teamMap : BW1058.getTrackingTeamMap(arena).entrySet()) {
