package club.mher.compass.menu.menus.bw1058.communicationMenus;
...
import club.mher.compass.menu.menus.bw1058.CommunicationsMenu;
...
public class TeamSelector extends Menu {
    ...
    public TeamSelector(BedWars bedWars, Player player, ITeam team, String path) {
        ...
        this.path = path;
        for (String s : BW1058.getMainConfig().getYml().getString(MainConfig.COMMUNICATIONS_MENU_TEAMS+".slots").split(",")) {
            int i;
            try {
    ...
    @Override
    public int getSlots() {
        return BW1058.getMainConfig().getInt(MainConfig.COMMUNICATIONS_MENU_TEAMS+".size");
    }
    ...
    public void setMenuItems() {
        ...
        for (ITeam t : arena.getTeams()) {
            ...
            inventory.setItem(slots.get(index), getTeamItem(BW1058.getMainConfig().getItem(player, MainConfig.TRACKER_MENU_TEAM_ITEM, false, null), t));
            teamMap.put(slots.get(index), t);
            index++;
        }
        NBTItem nbtBack = new NBTItem(bedWars, BW1058.getMainConfig().getItem(player, MainConfig.COMMUNICATIONS_MENU_TEAMS+".back-item", true, "back-item"));
